...
存在一个list\_a = [1, 2, 3, 4, 5, 6, 7, 8, 9]，现在需要逐一操作内部元素，并在操作结束之后删除它。使用\codeinline{python}{while}判断\codeinline{python}{list}是否为空，不为空则\codeinline{python}{pop}第一个元素，在循环下依次操作每一个元素。
